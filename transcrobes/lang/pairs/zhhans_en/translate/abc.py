...
class ZHHANS_EN_ABCDictTranslator(PersistenceProvider, Translator):
    ...
    def _load(self):
        ...
        logger.info("Finished populating abcdict, there are {} entries".format(len(list(dico.keys()))))
        return dico
    ...
    def get_standardised_defs(self, token):
        ...
        if entry:
            logger.debug("'{}' is in abcdict cache".format(token['lemma']))
            for abc in entry:
                logger.debug("Iterating on '{}''s different definitions in abcdict cache".format(token['lemma']))
        ...
        logger.debug("Finishing looking up '{}' in abcedict".format(token['lemma']))
        return std_format
