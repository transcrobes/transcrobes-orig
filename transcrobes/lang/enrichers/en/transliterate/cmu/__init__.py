...
from enrichers.models import EN_CMULookup
...
class CMU_EN_Transliterator(PersistenceProvider, Transliterator):
    """
    from enrichers.en.transliterate.cmu import CMU_EN_Transliterator; toto = CMU_EN_Transliterator({'path': '/opt/transcrobes/cmudict-0.7b.txt', 'inmem': True})
    """
    model_type = EN_CMULookup
    ...
    def _cmu_to_ipa(self, cmu_list, mark=True, stress_marking='all'):
    ...
    def _stress_type(self, stress):
        ...
        ## ORIGNAL - we want all so no point messing around
        ...
        #     return default
        # else:
        #     logger.warning(f"WARNING: stress type parameter {stress} not recognized.")
        #     # Use default stress
        #     return default
    ...
    # FIXME: rename variable `type`... honestly!
    def _find_stress(self, word, stype="all"):
                ...
                if c[-1] in stress_map.keys():
                    ...
                    if not new_word:
                        ...
                        for i, sym in enumerate(new_word):
                            ...
                            if CMU_PHONES[sym] in STOP_SET or (i > 0 and prev_phone == "stop") or sym in ["er", "w", "j"]:
    ...
    def transliterate(self, text):
        ...
        # the DB here
        return ' '.join(['/'.join(self.entry(t.lower()) or [f'{t}*']) for t in text.split()])
