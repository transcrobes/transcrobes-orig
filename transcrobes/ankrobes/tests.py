...
# from .views import add_note_chromecrobes, set_word_known, set_word
from ankrobes import Ankrobes, InvalidNoteFields
...
class AnkrobesTests(TestCase):
    ...
    def _set_word_known_tester(self, review_in, in_note):
        ...
        with Ankrobes(self.user.username) as userdb:
            status = userdb.set_word_known(simplified=in_note['Simplified'],
    ...
    def test_add_ankrobes_note_missing_fields(self):
        ...
        with Ankrobes(self.user.username) as userdb:
            with self.assertRaises(Exception):
                toto = userdb.add_ankrobes_note(simplified="",
                                                pinyin=in_note['Pinyin'],
                                                meanings=[in_note['Meaning']],
                                                tags=in_note['Tags'],
                                                review_in=0)
        ...
        with Ankrobes(self.user.username) as userdb:
            with self.assertRaises(Exception):
                toto = userdb.add_ankrobes_note(simplified=in_note['Simplified'],
                                                pinyin="",
                                                meanings=[in_note['Meaning']],
                                                tags=in_note['Tags'],
                                                review_in=0)
    ...
    def test_set_word_known_missing_fields(self):
        ...
        with Ankrobes(self.user.username) as userdb:
            with self.assertRaises(Exception):
                toto = userdb.set_word_known(simplified="",
                                                pinyin=in_note['Pinyin'],
                                                meanings=[in_note['Meaning']],
                                                tags=in_note['Tags'],
                                                review_in=1)
        ...
        with Ankrobes(self.user.username) as userdb:
            with self.assertRaises(Exception):
                toto = userdb.set_word_known(simplified=in_note['Simplified'],
                                                pinyin="",
                                                meanings=[in_note['Meaning']],
                                                tags=in_note['Tags'],
                                                review_in=1)
...
class EfficientConnectionTests(TestCase):
    ...
    def test_executemany(self, sql, l):
