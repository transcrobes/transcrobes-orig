...
class SocketCoreNLPProvider(ParseProvider):
    def parse(self, text):
        logger.debug('Starting SocketCoreNLPProvider parse of: {}'.format(text))
        ...
        for s in m['doc']['sentence']:
            ...
            sInd += 1
            for w in s['word']:
                sentence['tokens'].append({ 'index': wInd, 'word': w['lemma'],
                                           'originalText': w['originalText'], 'pos': w['pos'] })
                wInd += 1
...
class HTTPCoreNLPProvider(ParseProvider):
    def parse(self, text):
        logger.debug('Starting HTTPCoreNLPProvider parse of: {}'.format(text))
        ...
        logger.debug("Got the following back from CoreNLP via http: {}".format(r.text))
