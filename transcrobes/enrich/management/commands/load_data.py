...
DATA_PROVIDERS = {'en_zh_abc_dict': EN_ZHHANS_ABCDictTranslator,
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('-d', '--data_provider', action='append', nargs='+',
        ...
        parser.add_argument('-f', '--force_reload', default=False, action='store_true',
                            help='Default: False. Force reloading, even if there is already a '
                            'value in the DB for an entry')
    ...
    def handle(self, *args, **options):
        ...
        to_load = list([x] for x in DATA_PROVIDERS.keys()) if all_data else data_provider
        ...
        for prov in to_load:
            ...
            loader._load_to_db(loader._dict, force_reload)
