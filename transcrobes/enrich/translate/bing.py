...
class BingTranslator(Translator, BingAPI):
    ...
    def _ask_bing_lookup(self, content):
        found = BingAPILookup.objects.filter(source_text=content, from_lang=self.from_lang,
        ...
        if len(found) == 0:
            ...
            bing = BingAPILookup(source_text=content, response_json=bing_json, from_lang=self.from_lang,
                                 to_lang=self.to_lang)
    ...
    def _ask_bing_translate(self, content):
        found = BingAPITranslation.objects.filter(source_text=content, from_lang=self.from_lang,
                                                  to_lang=self.to_lang)
        logger.debug("Found {} elements in db for {}".format(len(found), content))
        if len(found) == 0:
            bing_json = self._ask_bing_api(content, TRANSLAT_PATH, self._translate_params())
            bing = BingAPITranslation(source_text=content, response_json=bing_json, from_lang=self.from_lang,
                                      to_lang=self.to_lang)
            bing.save()
            # with open(f'{bing.pk}.{content}.json', 'w') as bing_lookup:
            #     bing_lookup.write(bing_json)
