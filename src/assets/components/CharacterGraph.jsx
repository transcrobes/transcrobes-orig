...
export default class CharacterGraph extends Component {
  ...
  componentDidMount() {
    ...
    const { showAnswer, width, height, index } = this.props;
    ...
    const hanzi = HanziWriter.create(`${graph}-${index}`, graph, options);
    if (!showAnswer) {
      hanzi.quiz({
  ...
  render() {
    const { character, width, height, index } = this.props;
    const svgId = `${character.graph}-${index}`;
